...
2. 如果不存在，告知用户需要从 Chrome 导出 cookies：
   ...
   - **立即收紧权限**（cookies 等同于登录凭证）：`chmod 600 ~/cookies.json`
   - 然后终止流程，等用户完成后重新运行
...
# 只允许请求小红书域名，避免把登录 cookie 发到任意主机
post_url = '<帖子URL>'
host = urlparse(post_url).hostname or ''
if not (host == 'xiaohongshu.com' or host.endswith('.xiaohongshu.com')):
    raise ValueError(f'refusing non-xiaohongshu host: {host!r}')
...
# 保持 TLS 证书校验（默认行为）——绝不要禁用，否则 cookie 可被 MITM 窃取
...
#### 3b. 下载视频并提取音频
使用 `mktemp` 生成不可预测的私有临时文件名（避免 `/tmp` 下的 race / 信息泄露）：
```bash
XHS_TMPDIR=$(mktemp -d -t xhs.XXXXXXXX)
chmod 700 "$XHS_TMPDIR"
XHS_MP4="$XHS_TMPDIR/video.mp4"
XHS_WAV="$XHS_TMPDIR/audio.wav"

# --proto '=https' + --tlsv1.2 拒绝明文/旧协议；Referer 必须，否则返回 403
curl --fail --show-error --proto '=https' --tlsv1.2 -L \
     -H "Referer: https://www.xiaohongshu.com/" \
     -o "$XHS_MP4" "<视频URL>"
ffmpeg -y -i "$XHS_MP4" -vn -acodec pcm_s16le -ar 16000 -ac 1 "$XHS_WAV"
```

**注意：** `<视频URL>` 来自页面的 `__INITIAL_STATE__`（第三方内容）。在传给 `curl` 之前先 `urlparse`，只允许 `xhscdn.com` / `xiaohongshu.com` 等白名单主机，拒绝其他 host，防止被重定向到任意地址。
